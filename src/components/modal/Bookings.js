...
export const Bookings = ({ show, close, title }) => {
	...
	const handleDelete = (e) => {
		localStorage.clear();
		const newArray = reserva.filter(
			(reserva) =>
				`${reserva.originCity}-${reserva.destinyCity}` !==
				e.target.value
		);
		setReserva(newArray);
				...
				<div className="modalContainer" onClick={() => close()}>
					<div className="modal" onClick={(e) => e.stopPropagation()}>
										...
										<tr key={index}>
											...
											<td className="edit__booking">
												<button
													...
													value={`${reserva.originCity}-${reserva.destinyCity}`}
													onClick={(e) =>
														handleDelete(e)
													}
												>
						...
						<footer className="modal_footer">
							...
							<button className="submit" onClick={handleEmpty}>
								Vaciar carrito
							</button>
							<button
								className="submit"
